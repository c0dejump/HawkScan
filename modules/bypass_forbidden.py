...
def original_url(res, page, url):
	...
	if req_ou.status_code not in [403, 401, 404, 421, 429, 301, 302, 400, 408, 503, 405, 428, 412, 666, 500]:
...
def IP_authorization(res, url, domain, page):
	...
	for h in headers_type:
		for ip in ips_type:
			...
			if req_ip.status_code not in [403, 401, 404, 421, 429, 301, 302, 400, 408, 503, 405, 428, 412, 666, 500]:
...
def other_bypass(url, page, req_url):
	...
	for p in payl:
		...
		if req_payload.status_code not in [403, 401, 404, 421, 429, 301, 302, 400, 408, 503, 405, 428, 412, 666, 500] and len(req_payload.content) not in ranges:
