...
s,     """
    def check_multiple(self, s, req, res, directory, forbi, HOUR, parsing=False, size_bytes=False):
        """
        ...
        for m_exclude in req_p:
            try:
                if int(m_exclude):
                    #print(m_exclude)
                    check_code = self.check_exclude_code(s, res, req, directory, HOUR, parsing, multiple=True)
                    if check_code:
    ...
    def check_exclude_code(self, s, res, req, directory, HOUR, parsing=False, multiple=False):
        """
        check_exclude_code: 
            ...
            try:
                ...
                try:
                    ...
                    if status_link == 200:
                        ...
                        if "Generated by cloudfront" in req.text and "Request blocked" in req.text:
        ...
        if size_bytes == size_check or size_bytes in ranges:
            ...
            if exclude:
                if len(exclude) > 1:
                    ...
                    filterM.check_exclude_code(s, res_b, req_b, directory, HOUR, parsing)
                else:
            ...
            if exclude:
                if len(exclude) > 1:
                    ...
                    filterM.check_exclude_code(s, res, req_b, directory, HOUR, parsing)
                else:
                    filterM.check_exclude_page(s, req_b, res_b, directory, forbi, HOUR, parsing) 
            else:
...
def Progress(len_w, thread_count, nLine, page, percentage, tw, bp_current):
    ...
    """
    progress_print = "\033[34m 100% - {0}/{1} | T:{2} | Errors: {3} | CB: {4} | {5}\033[0m\r".format(n+nLine, len_w, thread_count, n_error, bp_current, page if len(page) < 60 else page.split("/")[-3:-1])
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='\033[32m Version 2.0 | contact: https://twitter.com/c0dejump\033[0m')
