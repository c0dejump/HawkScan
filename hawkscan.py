...
def auto_update():
    ...
    os.system("git status | tail -n1 > git_status.txt")
    with open("git_status.txt", "r+") as sg:
        for s in sg:
            print(s)
            if "nothing to commit" in s or "nothing added to commit" in s or "no changes added" in s:
